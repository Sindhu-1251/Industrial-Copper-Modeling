...
img = Image.open(r"C://Users//sindh//OneDrive//Desktop//Pro_img//download.png")
st.set_page_config(page_title= "Industrial Copper Modeling | By Sindhu S", page_icon=img,
                   layout= "wide",
...
with tab1:
        st.markdown(f'<h4 style=>This project aims to develop two machine learning models for the copper industry to address the challenges of predicting selling price and lead classification.</h4>',unsafe_allow_html=True)
        st.markdown(f'<h4 style=>Manual predictions can be time-consuming and may not result in optimal pricing decisions or accurately capture leads.</h4>',unsafe_allow_html=True)
        st.markdown(f'<h4 style=>The models will utilize advanced techniques such as data normalization, outlier detection and handling, handling data in the wrong format, identifying the distribution of features, and leveraging tree-based models, specifically the decision tree algorithm, to predict the selling price and leads accurately.</h4>',unsafe_allow_html=True)
        st.write(" ")
...
with tab2:
    ...
    if submit_button and flag==0:
            ...
            import pickle
            with open(r"R_model.pkl", 'rb') as file:
                loaded_model = pickle.load(file)
            with open(r"R_scaler.pkl", 'rb') as f:
                scaler_loaded = pickle.load(f)

            with open(r"R_type.pkl", 'rb') as f:
                t_loaded = pickle.load(f)

            with open(r"R_status.pkl", 'rb') as f:
...
with tab3: 
        ...
        if csubmit_button and cflag==0:
            import pickle
            with open(r"C_model.pkl", 'rb') as file:
                cloaded_model = pickle.load(file)

            with open(r"C_scaler.pkl", 'rb') as f:
                cscaler_loaded = pickle.load(f)

            with open(r"C_type.pkl", 'rb') as f:
